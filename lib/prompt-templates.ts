 ...
 */
export function buildBlendPrompt(hasShapeReference: boolean, hasCrossPhotoExemplar?: boolean): string {
  const draftImg = 1;
  let next = 2;
  const shapeImg = hasShapeReference ? next++ : null;
  const exemplarImg = hasCrossPhotoExemplar ? next++ : null;

  const lines = [
    `${draftImg}枚目の画像は、ヘアアクセサリーを既に正しい最終的な大きさ・位置に貼り付けた合成写真です（大きさ・位置は既に確定しており、変更の必要はありません）。`,
  ];

  if (shapeImg) {
    lines.push(
      `${shapeImg}枚目の画像は、そのアクセサリー単体を大きく写した「デザイン確認用」の資料です。${shapeImg}枚目はあくまで形・色を確認するためだけのものであり、完成写真に登場する被写体ではありません。`
    );
  }

  if (exemplarImg) {
    lines.push(
      `${exemplarImg}枚目の画像は、同じアクセサリーを別のモデル写真に正しい実物大で装着し、既に承認済みの完成例です。${draftImg}枚目のアクセサリーの最終的な大きさ感（頭・髪全体に対する比率）は、${exemplarImg}枚目に写っているものと同じになるようにしてください。ただし位置・向きは${draftImg}枚目の指定通りとし、${exemplarImg}枚目の背景やモデル・構図は一切参考にしないでください。`
    );
  }

  lines.push(
    `あなたの仕事は、${draftImg}枚目を自然な一枚の写真に見えるように仕上げることだけです：`,
  ...
  if (shapeImg) {
    lines.push(
      `・アクセサリーの細かい形（花や葉の形・配置・色）が${draftImg}枚目では小さくて潰れて見えても、${shapeImg}枚目を参考に、同じデザインを保ったまま正確に描いてください`
    );
  }

  if (shapeImg) {
    lines.push(
      `重要：完成写真に写るアクセサリーは、${draftImg}枚目に写っている、その1箇所・その大きさのものだけです。${shapeImg}枚目の画像そのもの、または${shapeImg}枚目のような大きな複製を、完成写真のどこにも新たに描き加えないでください。頭の中に2つ以上のアクセサリーが写ってはいけません。`
    );
  }

  lines.push(
    `アクセサリーの位置は${draftImg}枚目のまま絶対に変更しないでください。${
      exemplarImg
        ? `大きさは${exemplarImg}枚目の実例に合わせて調整して構いませんが、それ以外（向き・デザイン・色）は変更しないでください。`
        : "大きさ・向き・デザイン・色も変更しないでください。"
    }モデルの顔・髪型・肌・背景・衣装も変更しないでください。`,
    "コーム・ピン・クリップなどの装着部分（金属や樹脂の針金・土台）がいずれかの画像に写っていても、最終的な仕上がりには一切描かないでください。"
  );
